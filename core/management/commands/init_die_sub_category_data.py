...
class Command(BaseCommand):
    help = "Initialize master data for DieCategory from CSV file"

    def add_arguments(self, parser):
        parser.add_argument(
            "--csv-file",
            type=str,
            default="core/management/source/die_category.csv",
        ...
        try:
            with open(csv_file_path, "r", encoding="utf-8") as file:
                ...
                for row in reader:
                    ...
                    obj, created = DieCategory.objects.get_or_create(name=name)

                    if created:
                        created_count += 1
                        self.stdout.write(
                            self.style.SUCCESS(f"Created DieCategory: {name}")
                        )
                    else:
                        existing_count += 1
                        self.stdout.write(
                            self.style.WARNING(f"DieCategory already exists: {name}")
                        )
