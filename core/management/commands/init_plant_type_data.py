...
class Command(BaseCommand):
    help = "Initialize master data for PlantType from CSV file"

    def add_arguments(self, parser):
        parser.add_argument(
            "--csv-file",
            type=str,
            default="core/management/source/plant_type.csv",
        ...
        try:
            with open(csv_file_path, "r", encoding="utf-8") as file:
                ...
                for row in reader:
                    ...
                    # Create or get existing plant type
                    obj, created = PlantType.objects.get_or_create(
                        code=code,
                        defaults={"name": name, "status": status, "is_deleted": False},
                    )

                    if created:
                        created_count += 1
                        self.stdout.write(
                            self.style.SUCCESS(f"Created PlantType: {code} - {name}")
                        )
                    else:
                        existing_count += 1
                        self.stdout.write(
                            self.style.WARNING(
                                f"PlantType already exists: {code} - {name}"
                            )
                        )
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"\nPlant Type data initialization complete!\n"
