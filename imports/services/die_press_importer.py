...
class DiePressUniqueValidator(DatabaseUniqueValidator):
...
class DiePressImporter(BaseImporter):
    """
    Bulk importer for DiePress Master module
    """
    ...
    def get_validators(self) -> Dict[str, List]:
        ...
        return {
            ...
            "capacity": [
                IntegerValidator("capacity", required=False),
            ],
            "billet_diameter": [
                IntegerValidator("billet_diameter", required=False),
            ],
            "billet_length_min": [
                IntegerValidator("billet_length_min", required=False),
            ],
            "billet_length_max": [
                IntegerValidator("billet_length_max", required=False),
            ],
            "billet_weight": [
                DecimalValidator("billet_weight", required=False),
            ],
            "extrusion_length_min": [
                IntegerValidator("extrusion_length_min", required=False),
            ],
            "extrusion_length_max": [
                IntegerValidator("extrusion_length_max", required=False),
            ],
            "container_diameter": [
                DecimalValidator("container_diameter", required=False),
            ],
            "container_area": [
                DecimalValidator("container_area", required=False),
            ],
        }
        ...
        for col_name, field_name in field_mapping.items():
            ...
            if field_name in ["code", "name"]:
                ...
                if value is not None and value != "":
                    try:
                        transformed[field_name] = int(float(value))
    ...
    def create_model_instance(self, validated_data: Dict) -> DiePress:
        """
        Create DiePress model instance from validated data.

        Args:
            validated_data: Validated and transformed data

        Returns:
            DiePress instance (not saved)
        """
        return DiePress(**validated_data)
    ...
    def save_data(self) -> tuple[int, int, int, int, List[Dict]]:
        ...
        for data in self.validated_data:
            try:
                ...
                existing_records = DiePress.objects.filter(code=code, deleted=False)
