...
class DieGroupUniqueValidator(DatabaseUniqueValidator):
...
class DieGroupImporter(BaseImporter):
    """
    Bulk importer for DieGroup Master module
    """

    MODULE_NAME = "DieGroup"
    REQUIRED_COLUMNS = [
        "Name",
    ]
    ...
    def create_model_instance(self, validated_data: Dict) -> DieGroup:
        """
        Create a DieGroup model instance from validated data.

        Args:
            validated_data: Dictionary with validated field values

        Returns:
            DieGroup instance (not saved to DB yet)
        """
        return DieGroup(**validated_data)
    ...
    def save_data(self) -> tuple[int, int, int, int, List[Dict]]:
        ...
        for data in self.validated_data:
            try:
                ...
                existing_records = DieGroup.objects.filter(name=name, deleted=False)
