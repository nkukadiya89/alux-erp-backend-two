"""
PlantType Master bulk importer
"""
...
class PlantTypeImporter(BaseImporter):
    """
    Bulk importer for PlantType Master module
    """

    MODULE_NAME = "PlantType"
    REQUIRED_COLUMNS = [
        "Code",
        "Name",
    ]
    ...
    def get_validators(self) -> Dict[str, List]:
        ...
        return {
            "code": [
                # Removed UniqueValidator - uniqueness is handled in save_data method
                StringValidator("code", max_length=50, required=True),
            ],
            "name": [
                StringValidator("name", max_length=255, required=True),
            ],
            "status": [
                ChoiceValidator("status", PlantType.STATUS_CHOICES, required=False),
            ],
        }
    ...
    def transform_row_data(self, row_data: Dict) -> Dict:
        ...
        for col_name, field_name in field_mapping.items():
            ...
            if field_name == "code":
                ...
                # Map Status from CSV (Active/Inactive); default Active if empty
                transformed[field_name] = (
                    normalize_choice(value, PlantType.STATUS_CHOICES) or "Active"
                )
            else:
                transformed[field_name] = normalize_string(value) if value else None

        # Audit: only set created_by on create (updated_by/updated_at set only on update in save_data)
        transformed["created_by"] = self.user
        transformed["is_deleted"] = False

        return transformed

    def create_model_instance(self, validated_data: Dict) -> PlantType:
        """
        Create PlantType model instance from validated data.

        Args:
            validated_data: Validated and transformed data

        Returns:
            PlantType instance (not saved)
        """
        return PlantType(**validated_data)
    ...
    def save_data(self) -> tuple[int, int, int, int, List[Dict]]:
        ...
        for data in self.validated_data:
            try:
                ...
                # Check for existing records with the same code
                existing_records = PlantType.objects.filter(code=code, is_deleted=False)
                ...
                if existing_records.exists():
                    ...
                    if exact_duplicate_found:
                        ...
                        # No exact match found, but record with same code exists
                        # Update only business fields via QuerySet.update() so updated_at/updated_by are NOT set (bulk import)
                        ...
                        changed = any(
                            getattr(existing_to_update, k, None) != v
                    ...
                    # No record with this code exists - create new record
                    if not self.dry_run:
