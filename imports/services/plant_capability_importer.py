"""
PlantCapability Master bulk importer
"""
...
class PlantCapabilityImporter(BaseImporter):
    """
    Bulk importer for PlantCapability Master module
    """

    MODULE_NAME = "PlantCapability"
    REQUIRED_COLUMNS = [
        "Code",
        "Name",
    ]
    ...
    def create_model_instance(self, validated_data: Dict) -> PlantCapability:
        """
        Create PlantCapability model instance from validated data.

        Args:
            validated_data: Validated and transformed data

        Returns:
            PlantCapability instance (not saved)
        """
        return PlantCapability(**validated_data)
    ...
    def save_data(self) -> tuple[int, int, int, int, List[Dict]]:
        """
        Save validated PlantCapability rows.
        ...
        for data in self.validated_data:
            try:
                ...
                existing_records = PlantCapability.objects.filter(
                ...
                if existing_records.exists():
                    ...
                    if exact_duplicate_found:
                        ...
                        # Update the first existing record (there should only be one due to code uniqueness in practice)
