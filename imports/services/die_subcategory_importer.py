"""
DieSubCategory Master bulk importer
"""
...
class DieSubCategoryImporter(BaseImporter):
    """
    Bulk importer for DieSubCategory Master module
    """

    MODULE_NAME = "DieSubCategory"
    REQUIRED_COLUMNS = [
        "Name",
    ]
    ...
    def create_model_instance(self, validated_data: Dict) -> DieSubCategory:
        """
        Create DieSubCategory model instance from validated data.

        Args:
            validated_data: Validated and transformed data

        Returns:
            DieSubCategory instance (not saved)
        """
        return DieSubCategory(**validated_data)
    ...
    def save_data(self) -> tuple[int, int, int, int, List[Dict]]:
        """
        Save validated DieSubCategory rows.
        ...
        for data in self.validated_data:
            try:
                ...
                existing_records = DieSubCategory.objects.filter(
