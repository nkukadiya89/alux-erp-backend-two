...
from first_weight_entry.models import FirstWeightEntry

# Create your models here.


class SecondWeightEntry(models.Model):
    WEIGHT_FOR = [
    ...
    first_weight_entry = models.OneToOneField(
        FirstWeightEntry,
    ...
    is_same_weight_allowed = models.BooleanField(default=False)
    date_time_first = models.DateTimeField()
    date_time_second = models.DateTimeField(auto_now_add=True)
    mound = models.DecimalField(max_digits=50, decimal_places=3, null=True, blank=True)
    capture_photo_second_1 = models.CharField(max_length=500, null=True, blank=True)
    capture_photo_second_2 = models.CharField(max_length=500, null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.first_weight_entry:
            if self.weight_for == "gross weight" and not self.tare_weight:
                self.tare_weight = self.first_weight_entry.tare_weight

            elif self.weight_for == "tare weight" and not self.gross_weight:
                self.gross_weight = self.first_weight_entry.gross_weight
