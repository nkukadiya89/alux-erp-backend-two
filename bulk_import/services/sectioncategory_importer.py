...
class SectionCategoryImporter(BaseImporter):
    model = DieCategory
    unique_field = "name"
    ...
    def __init__(self, import_job_id: int):
        ...
        self.validators = [
            ...
            LengthFieldValidator({"name": {"max_length": 255}}),
        ]
    ...
    def process_rows(self, rows: List[Dict], user) -> Dict:
        ...
        for idx, row in enumerate(rows, start=2):
            try:
                ...
                try:
                    existing = DieCategory.objects.get(name=mapped["name"])
                    ...
                    new_instance = DieCategory.objects.create(**mapped)
                    result["inserted"] += 1
